...
global.ucooldown=1200; ucooldown=1200;
global.udesc=
"Borrows the power of stars...";
if instance_exists(obj_statboard) scr_Stats_Update(obj_statboard.button[0].show);
} else if global.uname=="TWILIGHT FURY" && !TWILIGHT_FURY {
global.uname="DUAL CLONE";
global.utype="BUFF";
global.ucooldown=600; ucooldown=600;
global.udesc=
...
if gpspeed!=0
if state==1
if !status_effect[4] && !status_effect[8] && !status_effect[10] {
if keyboard_check(global.keybind[5]) {bar_opacity[1]=5; if instance_exists(obj_evilflame_ultimate) obj_evilflame_ultimate.bar_opacity[1]=5; /*dual clone equivelent*/}
if keyboard_check_pressed(global.keybind[5]) && global.gpspeed_focus==1 focus_state=1;
if !keyboard_check(global.keybind[5]) && focus_state==1 focus_state=2;

if focus_state==1
{
if global.gpspeed_focus>0.25 global.gpspeed_focus=0.25;
else focus=home(focus,0,gpspeed/global.gpspeed_focus,0);
}
else if focus_state==2
{
if global.gpspeed_focus<1 global.gpspeed_focus=home(global.gpspeed_focus,1,0.05*gpspeed/global.gpspeed_focus,0);
else {focus_state=0;}
}
...
if focus<=0 && focus_state==1 focus_state=2;
}
...
if ultblink>0 ultblink--;
if flash_clock>0 flash_clock--;

/* */
///shooting & moving around
...
if(state==1 && gpspd!=0) {
    ...
    if(x==CANVAS_X+(sprite_get_xoffset(spr_evilflame_hitbox+boss.chrsel)-sprite_get_bbox_left(spr_evilflame_hitbox+boss.chrsel))*image_xscale or x==CANVAS_XEND+((sprite_get_xoffset(spr_evilflame_hitbox+boss.chrsel)-sprite_get_bbox_right(spr_evilflame_hitbox+boss.chrsel))*image_xscale)) {
        xv=0;
    }
    if(y==CANVAS_Y+(sprite_get_yoffset(spr_evilflame_hitbox+boss.chrsel)-sprite_get_bbox_top(spr_evilflame_hitbox+boss.chrsel))*image_yscale or y==CANVAS_YEND+((sprite_get_yoffset(spr_evilflame_hitbox+boss.chrsel)-sprite_get_bbox_bottom(spr_evilflame_hitbox+boss.chrsel))*image_yscale)) {
        yv=0;
    }
    
    if!(keyboard_check(global.keybind[0]) or keyboard_check(global.keybind[2])) {
        if(yv>0) {
    ...
    if!(keyboard_check(global.keybind[1]) or keyboard_check(global.keybind[3])) {
        if(xv>0) {
    ...
    //SHOOTING
    gpspd=gpspeed;
    var charge_sprite_lock=((charge>0 && artcharge==0) or ((boss.chrsel==1 or boss.chrsel==2) && instance_exists(obj_charge)));
    //can shoot condition:
    if((discharge>0 or !mouse_check_button(mb_right)) && !charge_sprite_lock && !((boss.chrsel==1) && status_effect[2])) {
        //is shooting event:
        if((keyboard_check(global.keybind[4]) or (mouse_check_button(mb_left) && (!place_meeting(boss.x,boss.y,obj_button) || instance_place(boss.x,boss.y,obj_button).image_alpha==0))) or status_effect[10]) {
            ...
            if(shot<=0) {
                ...
                switch(boss.chrsel)
                {
                    case 0: var xoffset=32*image_xscale; var yoffset=-7*image_yscale; break;
                    case 1: var xoffset=30*image_xscale; var yoffset=-19*image_yscale; break;
                    case 2: var xoffset=31*image_xscale; var yoffset=-17*image_yscale; var xoffset2=-39*image_xscale; var yoffset2=-4*image_yscale; break;
                }
                ...
                if(boss.chrsel==2) { //scootomik's second bullet
                    ...
                    if(x+xoffset2<CANVAS_X && instance_exists(helper)) {
                        var xx=helper.x;
                    } else {
                        var xx=x;
                    }
...
if(boss.chrsel==2 && state==1) {
    ...
    //CALCULATE WHEN AND WHERE TO PLACE HELPER
    if x+sprite_width-sprite_xoffset>CANVAS_XEND {
    if !instance_exists(obj_wrap_helper) {helper=instance_create(x-CANVAS_WIDTH,y,obj_wrap_helper); helper.mask_index=mask_index; helper.sprite_index=sprite_index; helper.image_index=image_index;}
    else
    {
    ...
    helper.image_xscale=image_xscale; helper.image_yscale=image_yscale;
...
&& !(boss.chrsel==1 && instance_exists(obj_charge)) //emerald's laser when it already exists
&& !((boss.chrsel==1) && status_effect[2]) //spell dried effect
{
//charging
bar_opacity[2]=5;
if discharge==0
...
} else if sprite_index<spr_evilflame_charging sprite_index=spr_evilflame_charging+boss.chrsel;
...
&& (!instance_exists(obj_charge) or (boss.chrsel!=1)) //emerald's laser
{if charge>=ctime {cb=1; play_sfx(sfx_evilflame_charge_shot+(boss.chrsel*4),sound_priority.player_charge_shot,0,sound_gpspeed*100); charge=0; if artcharge==0 discharge=ccooldown; else {artcharge=0; mouse_clear(mb_right);}}}

//resetting charging progress
if charge>0 && !mouse_check_button(mb_right) && gpspeed!=0 && charge<ctime {
charge=0;
bar_opacity[2]=0;
if evilflame_sprite_swap sprite_index=spr_evilflame_ultimate
else if evilflame_twilight_fury sprite_index=spr_evilflame_fury;
else sprite_index=spr_evilflame+boss.chrsel;
}
...
case 2:
cb=0;
if irandom(99)<30 {indicate(x,y,"FIZZLED!",2,c_white,c_yellow); sprite_index=spr_scootomik;}
else {spawn_bullet(x+20,y,obj_charge,boss.chrsel,0,-1,id);}
break;
default: spawn_bullet(x+20,y,obj_charge,boss.chrsel,0,-1,id); cb=0; break;
}
}
...
if(keyboard_check_pressed(global.keybind[6]) && ultcount>0
&& !status_effect[6] //ultimate cooldown
&& !(boss.chrsel==0 && (instance_exists(obj_evilflame_ultimate) or evilflame_twilight_fury==1))
&& !(boss.chrsel==1 && instance_exists(obj_emerald_ultimate))
&& !(boss.chrsel==2 && obj_player.status_effect[8])
&& state==1 && gpspeed!=0 && !instance_exists(obj_ultimate_activation)) {
    ultcount--;
...
//actual execution start
    ...
    ultblink=45;
        ...
        part_type_spawn_lt(global.part_system[5],global.player_part[0],0,x-sprite_xoffset,y-sprite_yoffset,x-sprite_xoffset+sprite_width,y-sprite_yoffset+sprite_height,"ellipse","invgaussian",100);
            if boss.chrsel==0 && !TWILIGHT_FURY { //evilflame's dual clone's burst
            ...
            else if boss.chrsel==1 { //emerald's current crush burst
...
if instance_exists(obj_wrap_helper) && instance_exists(helper) && place_meeting(helper.x,helper.y,obj_present) && instance_place(helper.x,helper.y,obj_present).picked==0
{
...
/* */
///contact & eprojectile damage (helper also)
if(state==1) {
    if(inv>0 && gpspeed!=0) {
        inv=max(inv-1,0);
    }
    if((boss.chrsel==2 && instance_exists(obj_charge) or instance_exists(obj_ultimate_activation)) && inv<=0) {
        inv=1;
    }
    ...
    if((place_meeting(x,y,obj_enemy) or place_meeting(x,y,obj_eprojectile)) && hp>0 && inv==0 && gpspeed!=0) {
        ...
        //contact damage
        if(place_meeting(x,y,obj_enemy) && (!place_meeting(x,y,obj_eprojectile) or (distance_to_object(instance_place(x,y,obj_eprojectile))>distance_to_object(instance_place(x,y,obj_enemy)))) && instance_place(x,y,obj_enemy).touchable==1 && instance_place(x,y,obj_enemy).hp>0) {
    ...
    if(instance_exists(obj_wrap_helper) && instance_exists(helper)) {
        ...
        if((place_meeting(hx,hy,obj_enemy) or place_meeting(hx,hy,obj_eprojectile)) && hp>0 && inv==0 && gpspeed!=0) {
            inv=round(invtime/gpspeed); play_sfx(sfx_player_hurt,0,0);
            //helper contact damage
            if(place_meeting(hx,hy,obj_enemy) && (!place_meeting(hx,hy,obj_eprojectile) or (distance_to_object(instance_place(hx,hy,obj_eprojectile))>distance_to_object(instance_place(hx,hy,obj_enemy)))) && instance_place(hx,hy,obj_enemy).touchable==1 && instance_place(hx,hy,obj_enemy).hp>0) {
            ...
            if(place_meeting(hx,hy,obj_eprojectile) && (!place_meeting(hx,hy,obj_enemy) or distance_to_object(instance_place(hx,hy,obj_enemy))>distance_to_object(instance_place(hx,hy,obj_eprojectile)) or instance_place(hx,hy,obj_enemy).touchable==0)) {
...
x=obj_evilflame_ultimate.x; y=obj_evilflame_ultimate.y;
hp=obj_evilflame_ultimate.hp; hpmax=obj_evilflame_ultimate.hpmax;
inv=obj_evilflame_ultimate.inv; invtime=obj_evilflame_ultimate.invtime;
hpmark=obj_evilflame_ultimate.hpmark; hpmark_v=obj_evilflame_ultimate.hpmark_v;
color=obj_evilflame_ultimate.color; shot=obj_evilflame_ultimate.shot;
charge=obj_evilflame_ultimate.charge; ctime=obj_evilflame_ultimate.ctime;
discharge=obj_evilflame_ultimate.discharge; ccooldown=obj_evilflame_ultimate.ccooldown;
artcharge=obj_evilflame_ultimate.artcharge; cb=obj_evilflame_ultimate.cb;
bar_opacity[0]=obj_evilflame_ultimate.bar_opacity[0]; bar_opacity[1]=obj_evilflame_ultimate.bar_opacity[1]; bar_opacity[2]=obj_evilflame_ultimate.bar_opacity[2];
bar_yoffset[0]=obj_evilflame_ultimate.bar_yoffset[0]; bar_yoffset[1]=obj_evilflame_ultimate.bar_yoffset[1]; bar_yoffset[2]=obj_evilflame_ultimate.bar_yoffset[2];
if evilflame_sprite_swap==1 {
if sprite_index==spr_evilflame sprite_index=spr_evilflame_ultimate;
else if sprite_index==spr_evilflame_shooting sprite_index=spr_evilflame_ultimate_shooting;
else if sprite_index==spr_evilflame_charging sprite_index=spr_evilflame_ultimate_charging;
} else {
if sprite_index==spr_evilflame_ultimate sprite_index=spr_evilflame;
else if sprite_index==spr_evilflame_ultimate_shooting sprite_index=spr_evilflame_shooting;
else if sprite_index==spr_evilflame_ultimate_charging sprite_index=spr_evilflame_charging;
}
with(obj_evilflame_ultimate) instance_destroy();
}
else {
image_alpha=0;
if boss.chrsel==1 with(obj_charge) {instance_destroy();}
if instance_exists(helper) with(helper) {instance_create(x,y,obj_explosion); instance_destroy();}
instance_create(x,y,obj_explosion);
state=2;
