...
///button interaction
if page>ceil((array_length_1d(slot)-1)/6) page=ceil((array_length_1d(slot)-1)/6);
...
if keyboard_check_pressed(vk_escape) or (boss.x>=x+284 && boss.x<=x+331 && boss.y>=y-240 && boss.y<=y-194 && grabbed==2 && mouse_check_button_released(mb_left))
{wipe(obj_slot); wipe(button[0]); wipe(button[1]); wipe(button[2]); wipe(button[3]); play_sfx(sfx_button3,0,0); instance_destroy();}
