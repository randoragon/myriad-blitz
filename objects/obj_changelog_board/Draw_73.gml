...
for(var i=0; i<array_length_1d(section); i++) {
    ...
    if(yy+(3*12)>=y-sprite_yoffset+50 && yy<=y-sprite_yoffset+sprite_height-70) {
        if(date[i]=="" or !(boss.x>=x-(0.5*3*string_width(string_hash_to_newline(section[i])))-2 && boss.x<=x+(0.5*3*string_width(string_hash_to_newline(section[i]))) && boss.y>yy-2 && boss.y<yy+(3*12)+2)) {
...
if(prevscroll+scrollv<-total_height or prevscroll+scrollv>0) {
    scrollv=0;
}

///draw cuts, tooltips && arrows
