...
if (global.shader_conditions == 0 || global.loading) {
    ...
    if (dizzy_alpha > 0) {
		...
		var dizzy_radius = 16;
        draw_surface_stretched(GENERAL_SURFACE, CANVAS_X - (2 * border) + ((dizzy_radius * dizzy_alpha) * dsin(gptime * 3)), CANVAS_Y - (2 * border) + ((dizzy_radius * dizzy_alpha) * dcos(gptime * 3)), window_get_width() + (2 * border), window_get_height() + (2 * border));
        draw_set_alpha(1);
    }
} else {
    ...
    if (dizzy_alpha > 0) {
        draw_set_alpha(dizzy_alpha);
        draw_surface_stretched(GENERAL_SURFACE, CANVAS_X - (2 * border) + ((16 * dizzy_alpha) * dsin(gptime * 3)), CANVAS_Y - (2 * border) + ((16 * dizzy_alpha) * dcos(gptime * 3)), window_get_width() + (2 * border), window_get_height() + (2 * border));
        draw_set_alpha(1);
    }
    shader_reset();
}
