...
if kill_count>=boss.last_ult+100 or (kill_count>boss.last_ult && irandom(99)==0)
{
loot[? "ultimate"]=1;
boss.last_ult+=100;
}
...
instance_setup(irandom(image_number),0.4*gpspeed);
break;
...
stats_projectile("p",-1,-1,-1);
instance_setup(irandom(image_number),0.5*gpspeed);
break;
...
instance_setup(irandom(image_number),0.2*gpspeed);
break;
...
instance_setup(irandom(image_number),random_range(0.15,0.2)*gpspeed);
y=clamp(y,100,556);
if(gpspeed!=0) {
    hspeed1=-25*image_speed/gpspeed;
} else {
    if(prev_gpspeed!=0) {
        hspeed1=-25*image_speed/prev_gpspeed;
    } else {
