...
///drawing self & overlays

//these are drawn elsewhere, so they skip this code block
if f==21 or f==22 {surface_reset_target(); exit;}
...
if sprite_index==spr_stats_checkbox && image_index=1
...
switch(f)
{
case 0:case 1:case 2:case 3:case 4: draw_sprite(spr_button_ol,f,x-(8*image_index)+((sprite_width-sprite_get_width(sprite_index))/2),y-(8*image_index)); break;
case 15: draw_sprite(spr_button_pause_ol,4,x,y); break;
case 9:case 10:case 11:case 12: draw_sprite(spr_button_pause_ol,f-9,x,y); break;
case 14: if(place_meeting(x,y,boss)) {draw_tooltip("Join Randoragon's#Discord server!",c_white,rgb(83,122,222),c_white);} break;
case 16: if(place_meeting(x,y,boss)) {draw_tooltip("Support Waterflame's#awesome music!",rgb(25,199,214),c_black,rgb(255,120,0));} break;
case 24: draw_set_align(fa_left,fa_middle); draw_set_color(c_black); draw_text_transformed(429,y,string_hash_to_newline("SAVE CHUNK SIZE"),3,3,0);
if(place_meeting(x,y,boss)) {draw_tooltip("Saving & loading will take longer, but#require less processing power.");} break;
case 25: if(place_meeting(x,y,boss)) {draw_tooltip("This is the default setting#sufficient for most machines.");} break;
case 26: if(place_meeting(x,y,boss)) {draw_tooltip("Sacrifices performance for more speed.#Works best on beefy computers.");} break;
case 27: if(place_meeting(x,y,boss)) {draw_tooltip("Check out my#other games!",rgb(128,255,0),c_dkgray,c_white);}
}
...
if(f==8 && instance_exists(obj_player) && global.loading==0) {
    ...
    if((ultblink>15 && ultblink<=30) or (ultblink<=-15 && ultblink>-30) or ultblink==0) {
    ...
    var color=global.color[boss.chrsel];
    ...
    if(instance_exists(obj_evilflame_ultimate) && boss.chrsel==0) { // EVILFLAME'S DUAL CLONE ALTERNATIVE
...
else draw_sprite(spr_status_exhausted+status_id,0,xx,yy);
...
if timer_countdown>=0 {
if timer_countdown<60 timer_text=" - "+string_format(timer_countdown/60,-1,2)+"s";
else if timer_countdown<3600 timer_text=" - "+string(floor(timer_countdown/60))+"s";
else timer_text=" - "+string(floor(timer_countdown/3600))+"m "+string(floor(timer_countdown/60)%60)+"s";
} else timer_text=" - "+chr(8734);
...
///death message, score, character names

if f==8 && room==rm_Main && !global.loading
{

//CHARACTER NAMES
if boss.transition!=1
{
draw_set_align(fa_center,fa_bottom);
draw_text_outline(GUI_X+683,GUI_Y+475,global.name[boss.chrsel],4-(4*boss.transition),4,0,c_black,1,c_black,1);
draw_text_outline(GUI_X+683,GUI_Y+471,global.name[boss.chrsel],4-(4*boss.transition),4,0,global.color[boss.chrsel],1,color_shift_hsv(global.color[boss.chrsel],0,0,-170,1),1);
}

//SCORE
draw_set_align(fa_right,fa_top);
draw_text_outline(GUI_XEND-10,GUI_Y-80+(85*boss.transition),"score: "+string_format(global.points,-1,0),3,3,0,c_yellow,1,c_black,1);
