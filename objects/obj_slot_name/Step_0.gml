...
if(keyboard_check_pressed(vk_escape) or (boss.x>=x+196 && boss.x<=x+243 && boss.y>=y-93 && boss.y<=y-49 && grabbed==2 && mouse_check_button_released(mb_left))) {
    instance_destroy();
}
...
if(keyboard_string!="") {
    ...
    if(keyboard_check_pressed(vk_enter) or (boss.x>=x-240 && boss.x<=x-192 && boss.y>=y+50 && boss.y<=y+195 && grabbed==1 && mouse_check_button_released(mb_left))) {
        switch(f)
        {
        case 0:
