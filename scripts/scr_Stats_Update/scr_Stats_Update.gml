/// @description scr_Stats_Update(show_all);
/// @param show_all
function scr_Stats_Update() {
	...
	with (obj_statboard) {
	    ...
	    for (var i = 0; i < a + 1; i++) {
	        if (section_length[i] > 0) {
	            linesum += linecount[i, section_length[i] - 1] + (datatype[i, section_length[i] - 1] == 1 || datatype[i, section_length[i] - 1] == 2);
	            count   += 1.2;
	        }
	    }
